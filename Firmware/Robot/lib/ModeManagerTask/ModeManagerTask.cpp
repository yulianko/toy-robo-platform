...
void ModeManagerTask::run() {
    showCurrentMode();
    ESP_LOGI(TAG, "Entering CHOOSING state");
    RobotEvent event;
...
void ModeManagerTask::returnToMenu() {
    ...
    ESP_LOGI(TAG, "[CHOOSING] exited '%s'", _modes[_activeIdx]->name());
...
void ModeManagerTask::showCurrentMode() {
    ...
    ESP_LOGI(TAG, "[MENU] Mode %d/%d: %s", _selectedIdx + 1, _modeCount, _modes[_selectedIdx]->name());
}
